...
class LogisticRegressor:
    ...
    def fit(
        ...
        # self.weights = np.array([0 for i in range(n)], dtype="float64")
    ...
    def predict_proba(self, X):
        ...
        # la prob es 1 / (1 + np.exp(-(self.bias+X*self.weights)))
        # y a la sigmoide metemos la cuota p / 1 - p
        z = self.bias + np.dot(X, self.weights) #esto es el w0 + x1*w1 + x2*w2 etc, y lo que metemos a la sigmoide
    ...
    def lasso_regularization(self, dw, m, C):
        ...
        lasso_gradient = (C / m) * np.sign(self.weights) #derivada del valor absoluto es el signo
    ...
    def ridge_regularization(self, dw, m, C):
        ...
        ridge_gradient =  (C/m) * self.weights #divimos entre m para normalizar, C es 1/landa
    ...
    def elasticnet_regularization(self, dw, m, C, l1_ratio):
        ...
        l1 = (C / m) * l1_ratio * np.sign(self.weights)
        l2 = (C / m) * (1 - l1_ratio) * self.weights
